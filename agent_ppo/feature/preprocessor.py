...
class Preprocessor:
    ...
    def _get_pos_feature(self, found, cur_pos, target_pos):
        ...
        target_pos_norm = norm(target_pos, 128, -128)#绝对坐标和相对坐标
        feature = np.array(
            (
                found,
    ...
    def process(self, terminated, truncated, obs, extra_info, rewardStateTracker, frame_state, last_action):
        ...
        return (
            feature,
            legal_action,
            reward_process(rewardStateTracker, game_info['step_no'], terminated, truncated, obs, frame_state[0], extra_info, frame_state[1], game_info['step_no'], self.feature_end_pos[-1], self.feature_history_pos[-1])
        )
